...
public class ModsCommand {
    ...
    private static int execute(@NotNull CommandContext<ServerCommandSource> context) {
        var mods = FabricLoader.getInstance().getAllMods().stream().filter(mod -> mod.getContainingMod().isEmpty()).map(mod -> mod.getMetadata().getId()).toList();
        var size = mods.size();
        context.getSource().sendFeedback(() -> Text.literal(String.format("Mods(%d): %s", size, String.join(", ", mods))), false);
        return size;
    }
}
